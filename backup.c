...
const double SCORE_WEIGHT = 2.0;
const double EMPTY_WEIGHT = 15.0;
const double DENSITY_WEIGHT = 20.0;
...
void predict(tauler& b, int passos) {
  ...
  if (passos == 0) return get_score(b, 1);
  ...
  for (int i=0; i<4; i++) {
    ...
    if (es_diferent(b, b1)) {
      scores.push_back(get_score(b1, 1)+predict);
    } else {
